...
const LawyerDashboard = () => {
  ...
  useEffect(() => {
    ...
    const storedDailyTime = JSON.parse(
    ...
    const interval = setInterval(() => {
      ...
      setDailyTimeSpent((prev) => {
        const updated = prev.map((d) =>
          d.date === today ? { ...d, hours: timeSpent } : d
        );
  ...
  useEffect(() => {
    ...
    const handleReceiveMessage = async ({
      ...
      if (!fetchedClientsRef.current.has(from)) {
        ...
        setChatClients((prev) =>
          prev.map((c) => (c._id === from ? { ...c, lastMessage: message } : c))
        );
      }
    };
  ...
  useEffect(() => {
    socket.on("typing", ({ fromUserType, name, fromUserId }) => {
      if (selectedClient?._id === fromUserId) {
        // Only show for current client
    ...
    try {
      ...
      setCallingData({
        ...
        callerInfo: {
          ...
          profilepic: selectedClient.profilepic,
        },
  ...
  const handleCallOptionSelect = (callType) => {
    if (selectedClient) {
      handleStartCall(selectedClient._id, callType);
    }
    handleCloseCallMenu();
  };
  ...
  const headerMenu = [{ label: "Notifications", path: "/notifications" }];
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setSelectedClient((prev) =>
                    prev && prev._id === clientId
                      ? prev
                      : chatClients.find((c) => c._id === clientId)
                  );
              ...
              <button
                ...
                onClick={() => {
                  ...
                  socket.emit("privateMessage", {
                    toUserId: clientId,
                    message:
                      "Sorry, the lawyer is busy now. Please try again later.",
                    fromUserType: "lawyer",
                    timestamp: new Date().toISOString(),
                  });
                }}
            ...
            >
              {chatClients?.map((client) => (
                <div
                  ...
                  className={`chat-tab ${
                    selectedClient?._id === client._id ? "active" : ""
                  }`}
                  style={{
                    padding: "0.5rem 1rem",
                    borderRadius: "20px",
                    ...
                    background:
                      selectedClient?._id === client._id
                        ...
                        : "#e5e7eb",
                    color:
                      selectedClient?._id === client._id ? "white" : "#374151",
                  }}
                >
                  {client.firstName}
                </div>
            ...
            >
              ...
              <input
                ...
                disabled={
                  !selectedClient ||
                  (selectedClient &&
                    (needsAccept?.[selectedClient._id] ||
                      !isSessionActive(selectedClient._id)))
                }
                style={{
                  width: "100%",
                  padding: "0.75rem 1rem",
              ...
              {selectedClient &&
                (needsAccept?.[selectedClient._id] ||
                  ...
                  <div style={{ color: "red", marginTop: 8, fontSize: 13 }}>
                    Accept the chat request to start/continue chatting with this
                    client.
                  </div>
                )}
